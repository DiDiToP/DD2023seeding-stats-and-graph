...
DataFile="Cx pH aSynPFF Noc.xlsx"
Xlabel1='C'
Xlabel2='CN'
Xlabel3="Sp"
Xlabel4="SpN"
Ylabel="Half-decay time (sec.)"


#symbols :  α, β


SaveAs='Cx pH aSynPFF Noc.png'
Columns=[0,1,2,3]
fig_dims = (1.2, 1.8)
#yaxisrange=0,5
fontsz = 8
...
g=sns.stripplot(data=df, palette=['#003366','#003366','#003366','#003366'], size=2, ax=g,zorder=3,  alpha=1)
...
ax.collections[3].set_transform(trans + offset)
...
g.set_yticks([0,10,20,30,40,50,60])
nobs=str(df[0].value_counts().sum())
nobs2=str(df[1].value_counts().sum())
nobs3=str(df[2].value_counts().sum())
nobs4=str(df[3].value_counts().sum())


g.set_xticklabels([Xlabel1, Xlabel2, Xlabel3, Xlabel4], fontsize=fontsz)
...
pvalueStat= StatTest(df[0],df[1],df[2],df[3]).pvalue
